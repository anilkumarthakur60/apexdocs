...
class ValidateCommand extends Command
{
    ...
    public function handle(ApexDocs $apexDocs): int
    {
        ...
        if ($this->errors === [] && ! ($strict && $this->warnings !== [])) {
            $this->info('<fg=green>✓</> Valid. '.count($this->warnings).' warning(s).');

            return self::SUCCESS;
        }

        $this->error(count($this->errors).' error(s), '.count($this->warnings).' warning(s).');
